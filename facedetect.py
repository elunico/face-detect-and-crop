...
def crop_to_boxes(filename: str, boxes: List[Tuple[int, int, int, int]], show: bool = False) -> 'List[cv2.image]':
    ...
    _on_each_box(name, ext, boxes, 'cropped', lambda img, x1, y1, x2, y2: img[y1:y2, x1:x2], show)

def draw_bounding_box(filename: str, boxes: List[Tuple[int, int, int, int]], show: bool = False) -> 'List[cv2.image]':
    '''
    This function takes a filename of an image and a list of bounding boxes
    ...
    _on_each_box(name, ext, boxes, 'marked', draw_rect, show)
...
def main_for_file(filename, drawOnly: bool = False, show: bool = False, limit: int = 5):
    ...
    if drawOnly:
        ...
        results = crop_to_boxes(filename, boxes, show)
    return results
